...
class Pet(Base):
 	"""
	domain model class for a Pet. Has a 
 	"""
 	__tablename__ = 'pet'
...
if __name__ == "__main__":
    ...
    # Now let's add two shelters
    log.info("Creating two shelters: NYC Pet Orphanage and New Orleans Pet Hotel")
    nyc = Shelter(name='NYC Pet Orphanage', website='http://www.nyc-pet-orphanage.com')
    new_orleans = Shelter(name='New Orleans Pet Hotel', website='http://www.neworleanspethotel.com')

    db_session.add_all([nyc, new_orleans])
    db_session.commit()

    log.info("New shelters saved. {} and {}".format(nyc.name, new_orleans.name))
    ...
    log.info('Now lets make two pets.')
    thomas = Pet(name='Thomas', age=5, adopted=False, breed_id= golden.id, shelter_id=nyc.id)
    sue = Pet(name='Sue', age=8, adopted=True, breed_id=poodle.id)

    db_session.add_all([thomas, sue])
    db_session.commit()

    log.info("We have {} pets!".format(len(db_session.query(Pet).all())))
    log.info("We have {} breeds!".format(len(db_session.query(Breed).all())))
    log.info("We have {} species!".format(len(db_session.query(Species).all())))
    #################################################
